...
import { flagCatalog, getCollection, getFlagsForCollection, learningCollections } from '../games/flaggenkunde/catalog.js'
...
export default function FlagQuizSetupPage() {
  ...
  const selectedItems = useMemo(() => getFlagsForCollection(selectedId), [selectedId])
  ...
  function selectCollection(id) {
    ...
    if (collection.quizMode === QUIZ_MODES.EUROPE_MAP) setQuizMode(QUIZ_MODES.EUROPE_MAP)
  ...
  function startQuiz() {
    const quiz = createQuiz(selectedItems, { collectionId: selectedId, roundLength: isHyperMode ? 'all' : roundLength, repeatMistakes: isMapMode ? false : repeatMistakes, quizMode })
      ...
      <main className="fq-setup-shell">
        ...
        <section aria-busy={!ready} className="fq-start-panel" aria-labelledby="fq-start-title">
          ...
          <div className="fq-library-heading">
            ...
            <div className="fq-library-filters">
          ...
          {[...groupedCollections].map(([group, collections]) => <div className="fq-collection-group" key={group}><h3>{group}</h3><div className="fq-collection-grid">{collections.map((collection) => <CollectionCard collection={collection} key={collection.id} onSelect={selectCollection} progress={progress} ready={ready} selected={selectedId === collection.id} />)}</div></div>)}
          {!filteredCollections.length ? <p className="fq-library-empty">Für diesen Filter gibt es noch keine eigene Sammlung.</p> : null}
